...
SavedSingleReport.saveReport = (data,result) => {
  //  console
  dbConn.query('INSERT INTO `client_session_report`(`session_id`, `pid`, `name`, `notes`,`status`, `timezone`) VALUES (?,?,?,?,?,?)', [data.session_id,data.pid,data.name,data.notes, data.status, data.timezone],  (err, res) => {
      if (err) {
        result(err ,null);
      } else {
        result(null, res)
